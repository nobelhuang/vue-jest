...
const compileTypescript = require('./compilers/jest-typescript-compiler')
...
function processScript (scriptPart, vueJestConfig, filePath, jestConfig, transformOptions) {
  if (!scriptPart) {
    return { code: '' }
  }

  if (/^typescript|tsx?$/.test(scriptPart.lang)) {
    return compileTypescript(scriptPart.content, vueJestConfig, filePath, jestConfig, transformOptions, scriptPart.lang)
  }
...
module.exports = function (src, filePath, jestConfig, transformOptions) {
  ...
  const result = processScript(parts.script, vueJestConfig, filePath, jestConfig, transformOptions)
